...
VENDOR_SIGNATURES: dict[str, list[tuple[re.Pattern[str], float]]] = {
    "TELUS": [(rx(r"\bTELUS\b"), 1.0), (rx(r"\btelus\.com\b"), 0.8), (rx(r"\bMy TELUS\b"), 0.8)],
    "BC HYDRO": [(rx(r"\bBC\s+Hydro\b"), 1.0), (rx(r"\bbchydro\.com\b"), 0.9), (rx(r"\bElectricity charges\b"), 0.4)],
    "FORTISBC": [(rx(r"\bFortisBC\b"), 1.0), (rx(r"\bfortisbc\.com\b"), 0.9), (rx(r"\bNatural gas\b"), 0.3)],
}
...
class InvoiceParser:
    ...
    account_patterns = [rx(r"(?:account|acct)\s*(?:number|no\.?|#)?\s*[:\-]?\s*" + ACCOUNT_CAPTURE)]
    ...
    previous_balance_patterns = [rx(r"\bprevious\s+balance\b\s*[:\-]?\s*" + MONEY_CAPTURE)]
    payment_patterns = [rx(r"\bpayments?(?:\s+received)?\b\s*[:\-]?\s*" + MONEY_CAPTURE)]
    meter_patterns = [rx(r"\bmeter\s*(?:number|no\.?|#)?\s*[:\-]?\s*([A-Z0-9][A-Z0-9\- ]{2,24})")]
    bill_date_patterns = [
    ...
    consumption_patterns = [rx(r"\b(?:usage|consumption)\b\s*[:\-]?\s*" + CONSUMPTION_CAPTURE)]
    ...
    def parse_account(self, raw: str) -> Optional[str]:
        generic = parse_identifier(raw)
        if generic is None:
            return None
        return canonical_account(self.vendor if self.vendor != "GENERIC" else "UNKNOWN", raw)

    def parse_meter(self, raw: str) -> Optional[str]:
        return normalize_meter(raw)

    def parse(self, view: DocumentView, profiles: Optional[dict[str, LayoutProfile]] = None) -> dict[str, FieldValue]:
        profiles = profiles or {}
        index = SpatialIndex(view.lines)
        fields: dict[str, FieldValue] = {}
        fields["account_number"] = best_field(
            line_pattern_extract(view.lines, self.account_patterns, self.parse_account),
            template_extract(index, profiles.get("account_number"), ACCOUNT_VALUE_RE, self.parse_account),
            spatial_extract(view.lines, index, self.account_anchors, ACCOUNT_VALUE_RE, self.parse_account),
        )
        ...
        fields["previous_balance"] = line_pattern_extract(view.lines, self.previous_balance_patterns, parse_money)
        fields["payments"] = line_pattern_extract(view.lines, self.payment_patterns, parse_money)
        fields["meter_number"] = best_field(
            line_pattern_extract(view.lines, self.meter_patterns, self.parse_meter),
            template_extract(index, profiles.get("meter_number"), METER_VALUE_RE, self.parse_meter),
            spatial_extract(view.lines, index, self.meter_anchors, METER_VALUE_RE, self.parse_meter),
        )
        ...
        fields["billing_period"] = line_pattern_extract(view.lines, self.billing_period_patterns, normalize_space, confidence=0.88)
        consumption, unit = self.extract_consumption(view)
        fields["consumption"] = consumption
        fields["consumption_unit"] = unit
        return fields

    def extract_consumption(self, view: DocumentView) -> tuple[FieldValue, FieldValue]:
        for line in view.lines:
            ...
            for pattern in self.consumption_patterns:
                ...
                if number is None:
                    continue
                number_fv = FieldValue(
                    value=number, raw=match.group(1), confidence=0.90, method="label_regex",
                    page=line.page + 1, evidence=text, x0=line.x0, y0=line.y0, x1=line.x1, y1=line.y1,
                    page_width=line.page_width, page_height=line.page_height,
                )
                unit_fv = FieldValue()
                if len(match.groups()) >= 2 and match.group(2):
                    unit_fv = FieldValue(
                        value=normalize_space(match.group(2)), raw=match.group(2), confidence=0.90, method="label_regex",
                        page=line.page + 1, evidence=text, x0=line.x0, y0=line.y0, x1=line.x1, y1=line.y1,
                        page_width=line.page_width, page_height=line.page_height,
                    )
                return number_fv, unit_fv
...
class TelusParser(InvoiceParser):
    vendor = "TELUS"
    VERSION = "4.0.0"
    account_patterns = [rx(r"\baccount\s*(?:number|#)?\s*[:\-]?\s*([0-9][0-9\- ]{5,20})")]
    current_charge_patterns = [
        rx(r"\bcurrent\s+charges?\b\s*[:\-]?\s*" + MONEY_CAPTURE),
...
class BCHydroParser(InvoiceParser):
    vendor = "BC HYDRO"
    VERSION = "4.0.0"
    # Historical BC Hydro PDFs may contain spaces inside the account number,
    # e.g. "902 741". Newer bills may show "902741". Both canonicalize to
    # the same string; the raw PDF representation remains in FieldValue.raw.
    account_patterns = [rx(r"\baccount\s*(?:number|no\.?|#)?\s*[:\-]?\s*([0-9][0-9 ]{4,22})")]
    meter_patterns = [rx(r"\bmeter\s*(?:number|no\.?|#)?\s*[:\-]?\s*([A-Z0-9][A-Z0-9\- ]{3,20})")]
    consumption_patterns = [rx(r"\b(?:electricity\s+)?(?:usage|consumption)\b\s*[:\-]?\s*([\d,]+(?:\.\d+)?)\s*(kWh|MWh)")]

    def parse_account(self, raw: str) -> Optional[str]:
        return canonical_account("BC HYDRO", raw)


class FortisBCParser(InvoiceParser):
    vendor = "FORTISBC"
    VERSION = "4.0.0"
    account_patterns = [rx(r"\baccount\s*(?:number|no\.?|#)?\s*[:\-]?\s*([0-9][0-9\- ]{5,22})")]
    consumption_patterns = [rx(r"\b(?:natural\s+gas\s+)?(?:usage|consumption)\b\s*[:\-]?\s*([\d,]+(?:\.\d+)?)\s*(GJ|m3|m³)")]


PARSER_MAP = {"TELUS": TelusParser, "BC HYDRO": BCHydroParser, "FORTISBC": FortisBCParser}
